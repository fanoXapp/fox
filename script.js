...
async function fetchSupabaseManifest(){
  ...
  // This works for both root-level videos and videos stored inside a folder such as "video-rells/".
  if(cfg.bucket){
    const prefixesToTry = [cfg.prefix || '', '', 'video-rells/', 'videos/'].filter((prefix, idx, arr) => arr.indexOf(prefix) === idx);
    for(const prefix of prefixesToTry){
      try{
        const listResp = await client.storage.from(cfg.bucket).list(prefix || '', { limit: 1000, offset: 0 });
        const listData = (listResp && listResp.data) ? listResp.data : listResp;
        console.log('fetchSupabaseManifest: storage.list response for prefix', prefix, {listResp, listData});
        if(listResp && listResp.error){
          console.warn('fetchSupabaseManifest: storage.list returned error for prefix', prefix, listResp.error);
          continue;
        }
        const entries = Array.isArray(listData) ? listData : (listData && Array.isArray(listData.data) ? listData.data : []);
        if(Array.isArray(entries) && entries.length){
          console.log('fetchSupabaseManifest: found', entries.length, 'items in bucket for prefix', prefix);
          const urls = entries.map((item, idx) => {
            const name = item && (item.name || item.path || item.id);
            if(!name) return null;
            const isVideo = /\.(mp4|mov|m4v|webm|mkv|ogg)$/i.test(name);
            if(!isVideo) return null;
            const safeName = String(name).replace(/^\//, '');
            const path = prefix ? `${prefix.replace(/\/$/, '')}/${safeName}` : safeName;
            const url = cfg.url.replace(/\/$/, '') + '/storage/v1/object/public/' + encodeURIComponent(cfg.bucket) + '/' + encodeURIComponent(path);
            if(idx < 3) console.log('  item[' + idx + '].name:', name, '→ url:', url);
            return url;
          }).filter(Boolean);
          if(urls.length){
            console.log('fetchSupabaseManifest: constructed', urls.length, 'public URLs from storage bucket', cfg.bucket, 'with prefix', prefix);
            return urls;
          }
        } else {
          console.log('fetchSupabaseManifest: storage.list returned empty or non-array for prefix', prefix, {isArray: Array.isArray(entries), length: entries ? entries.length : 'N/A'});
        }
      }catch(err){
        console.warn('fetchSupabaseManifest: error listing storage bucket for prefix', prefix, err.message || err);
      }
    }
